...
function logAccess($number, $fromBase, $toBase)
{
    try {
        ...
        $stmt->execute([
            'number' => $number, 'fromBase' => $fromBase, 'toBase' => $toBase,
            'ipaddr' => getUserIP()
        ]);
        $pdo->commit();
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') :
    ...
    if ($number !== $tmpnumber || $fromBase != $tmpfromBase || $toBase != $tmptoBase) {
        ...
        !is_numeric($fromBase) || (int)$fromBase <= 0 ||
        ...
        <div class="jumbotron">
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <p class="d-print-none"><a href="https://github.com/bytecodeman/baseconverter" target="_blank" rel="noopener noreferrer">Source Code</a></p>
                </div>
